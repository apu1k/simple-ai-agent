...
`WorkerExecutionPolicy.snapshot` copies approved, value-only repository, revision, image digest, provider/model IDs, profile, model settings, sandbox and persisted budget. `JobBudget` adds bounded request, input/output and command limits; older SQLite budget JSON loads defaults. The policy derives compatible turn/request and tool-call bounds for the executor and inference gateway; **no backend yet passes this policy to a live model or guest command session**. `ExecutionControl` observes a monotonic deadline/cancellation before and after synchronous calls, but cannot terminate a stalled provider SDK call or an uninterruptible tool transport. The current live preflight always refuses dispatch: without a reviewed interruptible inference transport, cleanup-safe job controller and retry-aware reservation, this is the only honest hard-cancellation/cost behavior. No daemon thread is used to claim an interrupted call has stopped.

Provider-neutral inference usage records total input/output tokens and optional cached input/reasoning output subsets (not additive), or explicit unknowns. Host OpenAI clients expose usage from the last SDK response; absent usage, empty-response retries, hidden SDK retries, failed calls and billing duplicates cannot be inferred as zero. Reporting usage is **not** hard spending enforcement. A strict USD budget is rejected before dispatch until an administrator-approved model/tier price table, finite token bounds and conservative per-attempt (including automatic retries) reservation exist. SDK requests currently use fixed 180-second/900-second Flex timeouts and unspecified automatic SDK retries, not a job's remaining deadline. `require_live_guarantees()` intentionally fails closed even for jobs without a USD ceiling. This groundwork and fake-clock tests do not pass Gate A/B or enable live jobs.
